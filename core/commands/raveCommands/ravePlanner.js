...
async function requestShotgun(raveKeyURL) {
  ...
  if (response.request.res.responseUrl != raveKeyURL) {
    ...
    newURL = tmpResponse.data.match(
      /window\.top\.location\.replace\(validateProtocol\(\"([^\"]+)/
    )[1];
...
async function removeSelectedRave(ctx) {
  ...
  for (let i = 0; i < raveList.length; i++) {
    if (raveList[i].url == raveUrl) {
      raveList.splice(i, 1);
      await writeToRaveFile();
      ctx.reply("Rave supprimée avec succès : " + raveUrl);
      return;
    }
  }
  ctx.reply(
    "Rave non trouvée - NB: vous devez utiliser l'url de la rave pour la supprimer !"
  );
}
...
function createEvent(shotgunEvent, attendingList = []) {
  return {
    ...
    location: {
      ...
      address:
        shotgunEvent.location.address.streetAddress ?? "Lieu non renseigné",
      url: shotgunEvent.location.address.url ?? "about:blank",
    },
    prices: shotgunEvent.offers
      .filter((offer) => offer.availability != "https://schema.org/SoldOut")
      .map((offer) => {
        return {
          price: offer.price,
...
async function updateRaveList(ctx, raveKeyURL, remove) {
  ...
  if (!remove) {
    try {
      if (
        raveKeyURL == undefined ||
        !raveKeyURL.match("https://[link.]*shotgun.live/.*")
      ) {
        ctx.reply(
          "Vous devez renseigner un lien Shotgun valide pour ajouter une rave !"
        );
