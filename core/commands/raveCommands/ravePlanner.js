...
async function writeToRaveFile() {
    fs.writeFileSync(`core/commands/raveCommands/ravePlanner.json`, JSON.stringify(raveList), async function (err) {
        if (err) {
            console.log(err);
        }
...
async function requestShotgun(raveKeyURL) {
    ...
    if (response.request.res.responseUrl != raveKeyURL) {
        ...
        newURL = tmpResponse.data.match(/window\.top\.location\.replace\(validateProtocol\(\"([^\"]+)/)[1];
...
function setGoingStatus(ctx, raveUrl) {
    for (let i = 0; i < raveList.length; i++) {
        if (raveList[i].url == raveUrl) {
            raveList[i].attending = raveList[i].attending.filter(user => user.id != ctx.update.message.from.id);
            writeToRaveFile();
            return;
        }
    }
